...
Searching the SMC API using these modules uses the smc-python `Collections Interface <http://smc-python.readthedocs.io/en/latest/pages/collections.html>`_. The format of a search query is determined by the element searched as well as filters that are used. It is also possible to specify case sensitivity, exact match, as well as limit the number of results.

Fact module arguments will have the same (or very close) signature:
...
**case_sensitive**: Turn on or off case sensitivity for the search. By default, case sensitivity is turned on.
...
**as_yaml**: Return the data in YAML format. Use a jinja template from the templates directory to format the YAML into your file of choice.
...
These examples show some simple searches.
...
Notice that in this filter, the element's `name` was specified. There may be cases where you do not have the name and instead want all entries with a given value. For example, show me all hosts with an IP address of '1.1.1.1':

You can just change the filter to the specific IP address that you are looking for to get those results:
...
Below is a list of fact modules currently provided for smc-ansible:
