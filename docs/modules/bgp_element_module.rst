...
.. raw:: html
    ...
    <td></td>
	<td>
        <p>List of device hashes or dictionaries with custom configurations based on the element type..</p>
	...
	<td>
        <p>Overwrite existing will replace the contents of the Access List element with the values provided in the element configuration. Otherwise, operations will be update_or_create, where an update will add new entries if they do not exist or fully create and add entries if the acl does not exist. To replace entries, you should fully define the access list and set overwrite_existing to true.</p>
	</td>
	</tr>
    </td>
    </tr>
